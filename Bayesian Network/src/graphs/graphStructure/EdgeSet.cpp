...
std::shared_ptr<NodeMap> EdgeSet::createNeighborhoodFromEdgeSet(std::shared_ptr<std::set<Edge>> edges) {
	...
	for (auto &edge : *edges) {
		updateNeighborhood(neighborhood, edge.getFirst(), edge.getSecond());

		updateNeighborhood(neighborhood, edge.getSecond(), edge.getFirst());
	}
...
std::shared_ptr<std::set<Edge>> EdgeSet::edgeSetDifference(std::shared_ptr<std::set<Edge>> edges1, std::shared_ptr<std::set<Edge>> edges2) {
	...
	for (auto &edge : *edges2) {
		auto edgeFound = edgeSetDiff->find(edge);
		if (edgeFound != edgeSetDiff->end())
			edgeSetDiff->erase(edgeFound);
	}
...
NodeMap::iterator EdgeSet::removeNode(const NodeId n) {
	...
	for (auto &node : m_neighbours.at(n)) {
		if(m_neighbours.at(node).find(n) != m_neighbours.at(node).end())
			m_neighbours.at(node).erase(n);
	}
...
std::shared_ptr<std::set<NodeId>> EdgeSet::verticesFromEdges(std::shared_ptr<std::set<Edge>> edges) {
	...
	for (auto &edge : *edges) {
		vertices->insert(edge.getFirst());
		vertices->insert(edge.getSecond());
	}
